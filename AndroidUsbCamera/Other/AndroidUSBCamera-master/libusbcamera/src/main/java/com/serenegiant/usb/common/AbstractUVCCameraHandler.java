        ...
        if (isPreviewing()) {
            ...
            synchronized (thread.mSync) {
                ...
                if (!isCameraThread()) {
                    ...
                    try {
                        thread.mSync.wait();
                        Log.i(TAG, "stopPreview, thread.mSync.wait...finish");
                    } catch (final InterruptedException e) {
                        Log.i(TAG, "stopPreview, thread.mSync.wait...error");
                        e.printStackTrace();
                    }
                }
            }
    ...
    public void handleMessage(final Message msg) {
        ...
        switch (msg.what) {
            ...
            case MSG_PREVIEW_STOP:
                Log.i(TAG, "handleMessage.MSG_PREVIEW_STOP");
...
//				mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_NV21);
        ...
        public void handleStopPreview() {
            if (DEBUG)
                Log.v(TAG_THREAD, "handleStopPreview:" + mIsPreviewing + ", " + (mUVCCamera != null));
            if (mIsPreviewing) {
                if (mUVCCamera != null) {
                    mUVCCamera.stopPreview();
                    Log.v(TAG_THREAD, "handleStopPreview:stopPreview:");
                    mUVCCamera.setFrameCallback(null, 0);
                }
                if (DEBUG) Log.v(TAG_THREAD, "handleStopPreview:toSynchronized");
                synchronized (mSync) {
                    mIsPreviewing = false;
                    mSync.notifyAll();
                }
