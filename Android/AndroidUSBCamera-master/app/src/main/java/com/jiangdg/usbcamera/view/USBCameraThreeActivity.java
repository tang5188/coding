...
public class USBCameraThreeActivity extends AppCompatActivity {
    ...
    public static final String CameraName3 = "XCX-S58M21";

    private TextView tvCamera1, tvCamera2, tvCamera3;
    private UVCCameraTextureView mTexture1, mTexture2, mTexture3;

    private USBMonitor usbMonitor;
    private UVCCamera uvcCamera1, uvcCamera2, uvcCamera3;
    private boolean isPreview1, isPreview2, isPreview3;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_usbcamera_three);

        tvCamera1 = (TextView) findViewById(R.id.tvCamera1);
        tvCamera2 = (TextView) findViewById(R.id.tvCamera2);
        tvCamera3 = (TextView) findViewById(R.id.tvCamera3);
        ...
        mTexture3 = (UVCCameraTextureView) findViewById(R.id.camera_view3);

        uvcCamera1 = new UVCCamera();
        uvcCamera2 = new UVCCamera();
        uvcCamera3 = new UVCCamera();
        ...
        usbMonitor = new USBMonitor(this, new USBMonitor.OnDeviceConnectListener() {
            ...
            public void onAttach(final UsbDevice device) {
                ...
                if (device.getDeviceClass() == 239 && device.getDeviceSubclass() == 2) {//根据相机信息选择选需要打开的相机
                    if (device.getProductName().contains(CameraName1) ||
                            ...
                            device.getProductName().contains(CameraName3)) {
                        ...
                        if (device.getProductName().contains(CameraName3) && isPreview3) return;
            ...
            public void onConnect(UsbDevice device, USBMonitor.UsbControlBlock ctrlBlock, boolean createNew) {
                if (device.getProductName().contains(CameraName1) ||
                    ...
                    new Thread(new USBCameraThreeActivity.myRunnable(device, ctrlBlock)).start();
                }
            }
            ...
            public void onDisconnect(UsbDevice device, USBMonitor.UsbControlBlock ctrlBlock) {
                ...
                if (device.getProductName().contains(CameraName1)) {
                    ...
                    isPreview2 = false;
                    tvCamera2.setText("");
                } else if (device.getProductName().contains(CameraName3)) {
                    isPreview3 = false;
                    tvCamera3.setText("");
                }
            }
        ...
        ((Switch) findViewById(R.id.switch3)).setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b) {
                    uvcCamera3.setPreviewTexture(mTexture3.getSurfaceTexture());
                    uvcCamera3.startPreview();
                } else {
                    uvcCamera3.stopPreview();
                }
            }
        });
    }
    ...
    class myRunnable implements Runnable {
        ...
        public void run() {
            if (usbControlBlock.getProductName().contains(CameraName1)) {
                ...
                camera = uvcCamera2;
            } else if (usbControlBlock.getProductName().contains(CameraName3)) {
                camera = uvcCamera3;
            }
            ...
            if (usbControlBlock.getProductName().contains(CameraName1)) {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        tvCamera3.setText(device.getProductName() + ", " + camera.getDeviceName());
                    }
                });
            }
            camera.startPreview();
    ...
    private void setCameraParameter(UsbDevice device, UVCCamera camera) {
        try {
            ...
            if (device.getProductName().equals(CameraName1)) {
                camera.setPreviewSize(1280, 720, 1, 31, UVCCamera.FRAME_FORMAT_MJPEG, 0.33f);
                ...
                camera.setPreviewSize(1280, 1024, 1, 31, UVCCamera.FRAME_FORMAT_MJPEG, 0.33f);
                Log.d(TAG, "**设置参数成功2=FRAME_FORMAT_MJPEG, " + device.getProductName() + ", " + camera.getDeviceName());
            } else if (device.getProductName().equals(CameraName3)) {
                camera.setPreviewSize(640, 480, 1, 31, UVCCamera.FRAME_FORMAT_MJPEG, 0.33f);
                Log.d(TAG, "**设置参数成功3=FRAME_FORMAT_MJPEG, " + device.getProductName() + ", " + camera.getDeviceName());
            }
